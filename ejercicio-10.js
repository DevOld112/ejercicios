...
const generateDNI = (numDNI) => {

    //Definir las letras de DNI

    const letras = ['T', 'R', 'W', 'A', 'G', 'M', 'Y', 'F', 'P', 'D', 'X', 
                'B', 'N', 'J', 'Z', 'S', 'Q', 'V', 'H', 'L', 'C', 'K', 'E', 'T'];

    //Convertir el numero de DNI

    numDNI = parseInt(numDNI)

    //Validar si la variable ingresada es valida

    if(numDNI > 0 && numDNI < 99999999){
        let index = numDNI % 23
        let letter = letras[index]

        return numDNI.toString() + letter
    } else {
        return 'DNI INVALIDO'
    }
...
console.log(generateDNI("81536272"))